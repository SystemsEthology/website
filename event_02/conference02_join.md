---
layout: page_conference_02
title: 参加登録・要旨登録
order: 5
ogp_image: /event_02/images/systems_ethology_2026_01.png
---
***

### 参加登録：締切 2026年7月10日 23:59
>- 参加者人数把握のため、お早めの参加登録をお願いいたします。
>- 懇親会の人数によっては参加登録締め切り前に締め切る可能性がございます。ご了承ください。
>- 発表を希望される方は6/10までに参加登録・要旨登録の両方を行なってください。

<br>

### 参加費

| 区分 | 参加費 |
| :--- | ---: |
| 学生 (現地参加) | 1,500円 |
| 一般 (現地参加) | 6,000円 |
| 学生 (オンライン参加) | 0円 |
| 一般 (オンライン参加) | 1,000円 |

<br>
<br>

>- 早割で参加登録された方は (クラウドファンディング支援者様)の項目をご選択ください。
>- 参加費には、ミキサー(1日目)、休憩時間やポスターセッションでの軽食やコーヒー　が含まれます。
>- 懇親会(2日目)は含まれません。下記の懇親会費よりお申し込みください。

<br>
<br>

### 懇親会費

| 区分 | 参加費 |
| :--- | ---: |
| 学生 | 1,000円 |
| 一般 | 5,000円 |

<br>
<br>
<a href="https://app.payvent.net/embedded_forms/show/69e851cd68ada6e44ca651fb" target="_blank" rel="noopener noreferrer">
  参加申し込みフォーム
</a>

<br>

### 発表者　要旨登録：締切 2026年6月10日 23:59

<br>
<a href="https://app.payvent.net/embedded_forms/show/69e852de68ada6e44da827dc" target="_blank" rel="noopener noreferrer">
  要旨登録フォーム
</a>



>- 参加登録フォームとは別になっております。発表を希望される方はこちらも期日までにご登録お願い致します。

<br>
<br>

***

### 現在の参加者数（5月23日現在）：40人　ベータ版キーワードマップ

<div style="width:100%; height:650px; border-radius:10px; overflow:hidden; box-shadow:0 1px 4px rgba(0,0,0,.12);">
  <iframe
    src="{{ '/event_02/mapping.html' | relative_url }}"
    style="width:100%; height:100%; border:none;"
    loading="lazy"
    title="参加者キーワードマップ">
  </iframe>
</div>
